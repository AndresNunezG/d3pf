<template>
    <div class="artisan-item d-flex bg-dark p-3 mb-2 rounded">
        <div class="icon d-flex justify-content-center align-items-center rounded-circle mr-2">
            ...
            <font-awesome-icon :icon="artisan.icon" class="fa-2x" :style="{color: artisan.color}"></font-awesome-icon>
        </div>
        <div class="content">
            <h5 class="font-weight-bold text-capitalize">{{ artisan.name }}</h5>
            <p v-if="artisan.normal.level" class="m-0 font-weight-normal">Level {{ artisan.normal.level }} (normal)</p>
